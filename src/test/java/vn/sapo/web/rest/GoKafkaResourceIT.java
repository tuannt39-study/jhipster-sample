...
import vn.sapo.config.KafkaProperties;

class GoKafkaResourceIT {
    ...
    void setup() {
        ...
        GoKafkaResource kafkaResource = new GoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/go-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/go-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
